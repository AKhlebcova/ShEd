...
def import_data(shapefile):
    ...
    for suffix in required_suffixes:
        if not has_suffix[suffix]:
            zip.close()
            os.remove(fname)
            return f"Archive missing required {suffix} file."
    ...
    for info in zip.infolist():
        ...
        for i in required_suffixes:
            if info.filename.endswith(i):
    ...
    # print(len(src_spatial_ref.ExportToWkt()))
    # print(len(geom_name))
    ...
    for i in range(layer_def.GetFieldCount()):
        ...
        # print(field_def.GetFieldTypeName(field_def.GetType()))
        # print(oft_fields.get(field_def.GetType(), 'unknown'))
        attr.save()
...
def export_data(shapefile):
    ...
    # return HttpResponse("Продолжение следует...")
